...
async def find_paper_by_id(client, paper_id: int):
    try:
        response = await client.search(
            index="your_index_name", body={"query": {"term": {"paper_id": paper_id}}}
        )

        # Process results
        hits = response["hits"]["hits"]
        if hits:
            return hits[0]["_source"]  # Return first match
        else:
            return None

    except Exception as e:
        logger.error(f"Failed to search paper_id {paper_id}: {e}")


async def query_predictions(client: AsyncOpenSearch, index_name="pred-result"):
    """Run sample queries on the prediction data"""

    logger.info("Running sample queries on prediction data...")
    ...
    try:
        stats_query = {
            "size": 0,
            "aggs": {
                "prediction_stats": {"stats": {"field": "prediction"}},
                "paper_count": {"value_count": {"field": "paper_id"}},
            },
        }

        response = await client.search(index=index_name, body=stats_query)

        pred_stats = response["aggregations"]["prediction_stats"]
        total_papers = response["aggregations"]["paper_count"]["value"]

        logger.info(" Dataset Statistics:")
        logger.info(f"   • Total papers: {total_papers}")
        logger.info(
            f"   • Prediction range: {pred_stats['min']:.3f} - {pred_stats['max']:.3f}"
        )
        logger.info(f"   • Average prediction: {pred_stats['avg']:.3f}")
